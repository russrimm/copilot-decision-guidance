...
export async function generateUseCasePDF(useCase: UseCase): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    ['phase1', 'phase2', 'phase3'].forEach((phase, idx) => {
      ...
      const phaseData = useCase.implementation[
        phase as keyof typeof useCase.implementation
      ] as string[];
...
export async function generateMultipleUseCasesPDF(useCases: UseCase[]): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      size: 'A4',
      margin: 50,
    ...
    doc
      .fontSize(11)
      .text(
        'This document contains detailed information about AI-powered use cases tailored to your organization. Each use case includes implementation guidance, architecture details, ROI analysis, and next steps.',
    ...
    useCases.forEach((useCase, idx) => {
      ...
      doc.text(
        `Timeline: ${useCase.implementation.estimatedTimelineWeeks} weeks | Payback: ${useCase.roi.paybackPeriodMonths} months | Value: ${useCase.roi.estimatedAnnualValue}`,
        { width: 500 }
      );
      doc.moveDown(0.5);
