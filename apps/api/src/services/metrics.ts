...
interface MetricsConfig {
  ...
  enableCostReports?: boolean;
  enablePowerPlatform?: boolean;
  enableLicenses?: boolean;
  enableSecureScore?: boolean;
}
...
interface PowerPlatformInventory {
...
interface LicenseCount {
  ...
  licenses: {
    ...
    assigned: number;
    available: number;
    relevant?: boolean; // Copilot-related licenses
  }[];
  copilotLicenses: {
    m365Copilot: number;
...
export class MetricsService {
  ...
  async getCostReport(): Promise<CostReport | null> {
    ...
    try {
      ...
      // Parse cost data
      const breakdown =
        data.properties?.rows?.map((row: any[]) => ({
          service: row[1] || 'Unknown',
          cost: parseFloat(row[0]) || 0,
        })) || [];

      const totalCost = breakdown.reduce((sum: number, item: any) => sum + item.cost, 0);

      // Try to identify Copilot-related costs
      const copilotRelatedCosts = breakdown
        .filter(
          (item: any) =>
            item.service.toLowerCase().includes('cognitive') ||
   ...
   */
  async getPowerPlatformInventory(): Promise<PowerPlatformInventory | null> {
   ...
   */
  async getLicenseCounts(): Promise<LicenseCount | null> {
    ...
    try {
      ...
      const m365CopilotCount = licenses
        .filter(
          (l: any) =>
            l.skuName.includes('M365_COPILOT') || l.skuName.includes('MICROSOFT_365_COPILOT')
        )
        .reduce((sum: number, l: any) => sum + l.assigned, 0);

      const copilotStudioCount = licenses
        .filter((l: any) => l.skuName.includes('COPILOT_STUDIO'))
        .reduce((sum: number, l: any) => sum + l.assigned, 0);

      const powerAutomateCount = licenses
        .filter((l: any) => l.skuName.includes('POWER_AUTOMATE') || l.skuName.includes('FLOW'))
        .reduce((sum: number, l: any) => sum + l.assigned, 0);

      return {
        totalLicenses: licenses.reduce((sum: number, l: any) => sum + l.assigned, 0),
        licenses: licenses.filter((l: any) => l.relevant).slice(0, 20),
        copilotLicenses: {
  ...
  private isCopilotRelatedLicense(skuName: string): boolean {
  ...
  async getSecureScore(): Promise<SecureScore | null> {
    ...
    try {
      ...
      const recResponse = await fetch(
        "https://graph.microsoft.com/v1.0/security/secureScoreControlProfiles?$top=10&$filter=actionType eq 'Review'",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );

      const recData = recResponse.ok ? ((await recResponse.json()) as any) : ({ value: [] } as any);
  ...
  private calculateReadinessScore(
    ...
    if (metrics.powerPlatform) {
      ...
      if (metrics.powerPlatform.totalEnvironments > 0) technical += 10;
    }
    ...
    if (metrics.licenses) {
      if (metrics.licenses.copilotLicenses.m365Copilot > 0) technical += 10;
      if (metrics.licenses.copilotLicenses.copilotStudio > 0) technical += 10;
    }
   ...
   */
  async getAllMetrics(): Promise<TenantMetrics> {
    const metrics: Partial<TenantMetrics> = {
    ...
    return metrics as TenantMetrics;
  }

  /**
   * Get demo/sample data when metrics are not configured
   */
  static getSampleMetrics(): TenantMetrics {
    return {
      timestamp: new Date().toISOString(),
...
export type { TenantMetrics, CostReport, PowerPlatformInventory, LicenseCount, SecureScore };
