...
export class AIRecommendationService {
            ...
            const response = await fetch(this.endpoint, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an expert Microsoft solutions architect specializing in AI and agentic platforms. You provide strategic, actionable recommendations for organizations evaluating Microsoft Copilot products.',
                        },
                        {
                            role: 'user',
                            content: prompt,
    ...
    parseAIResponse(aiResponse, request) {
        ...
        for (const section of sections) {
            ...
            else if (section.includes('IMPLEMENTATION ROADMAP') ||
            ...
            else if (section.includes('COST CONSIDERATIONS') ||
     ...
     */
    extractPersonalizationFactors(request) {
        const factors = [];
        ...
        if (tenantMetrics?.powerPlatform) {
            if (tenantMetrics.powerPlatform.totalCopilotStudioAgents > 0) {
                factors.push(`${tenantMetrics.powerPlatform.totalCopilotStudioAgents} Copilot Studio agents in use`);
            }
     ...
     */
    generateFallbackRecommendation(request) {
