...
function buildExecutiveOverviewContent(input) {
    ...
    const removeOverlap = (items) => items.filter((item) => !referenceSignals.has(normalizeForCompare(item)));
    return {
        strategicContext: input.aiRecommendation?.strategicGuidance ||
            input.aiRecommendation?.summary ||
            input.recommendation.summary ||
            'Agentic AI changes how work gets done by shifting teams from task execution to outcome ownership.',
        principle: 'AI does not replace humans; it amplifies human creativity, judgement, and collaboration.',
...
export async function generateExecutiveOverviewPPTX(input) {
    ...
    const aiClarifications = input.aiRecommendation?.clarifications?.filter((c) => (c ?? '').trim().length > 0) ?? [];
    if (aiClarifications.length > 0) {
        const sClar = prs.addSlide();
        addTitle(sClar, 'Key Clarifications');
        sClar.addText('Common misconceptions corrected — to help you apply this recommendation accurately.', {
            x: 0.6,
            y: 1.2,
            w: 8.8,
            h: 0.35,
