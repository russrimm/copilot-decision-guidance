...
const DEPARTMENT_TOPIC_SLIDE_DATA = [
    {
...
function bulletize(items, maxItems = 5) {
    return items
        .slice(0, maxItems)
        .map((item) => `• ${item}`)
        .join('\n');
...
function bufferFromPptOutput(output) {
...
function addDepartmentTopicsSlide(prs) {
...
export async function generateUseCasePPTX(useCase) {
    ...
    slide4.addText(`Estimated timeline: ${useCase.implementation.estimatedTimelineWeeks} weeks  |  Typical phase duration: ~${phaseDuration} weeks  |  Skills: ${useCase.implementation.skillsRequired.slice(0, 5).join(', ')}`, {
        x: 0.6,
        y: 6.55,
        w: 11.9,
        h: 0.35,
...
export async function generateMultipleUseCasesPPTX(useCases) {
    ...
    const verticalCount = useCases.reduce((acc, useCase) => {
        acc[useCase.vertical] = (acc[useCase.vertical] ?? 0) + 1;
        return acc;
    }, {});
    slide2.addText(`Vertical coverage: ${Object.entries(verticalCount)
    ...
    useCases.forEach((useCase, index) => {
        ...
        slide2.addText(`${useCase.vertical.toUpperCase()} | ${useCase.implementation.estimatedTimelineWeeks}w | ${useCase.roi.paybackPeriodMonths}m payback`, {
            x: 1,
            y: listY + 0.4,
            w: 6.7,
            h: 0.2,
    ...
    useCases.forEach((useCase, index) => {
        ...
        slide.addText(bulletize([...useCase.implementation.phase1, ...useCase.implementation.phase2], 5), {
            x: 0.85,
            y: 4.33,
            w: 5.3,
            h: 1.92,
