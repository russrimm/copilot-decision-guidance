...
import { checkImplementationGuideUpdate, getImplementationGuideMetadata, getImplementationGuideUpdateHistory, } from './services/implementation-guide-monitor.js';
console.log('[2/10] Core imports loaded successfully');
let decisionModel;
let calculateRecommendation;
let generateRecommendation;
let UserAnswersSchema;
let MetricsService;
...
const licensingDataPromise = readFile(path.join(__dirname, '../../../packages/decision-engine/dist/data/licensing-data.json'), 'utf-8').then((content) => JSON.parse(content));
...
const isChatHistory = (value) => Array.isArray(value) &&
    value.length <= 10 &&
...
const readinessQuestions = [
    {
        ...
        examples: {
            ...
            watch: 'Conditional Access exists but excludes service accounts or pilot groups that can still sign in without MFA.',
            highRisk: 'No Conditional Access policy protects pilot users, or broad exclusions allow unmanaged device sign-in.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Roles are defined, but periodic access reviews and privileged role controls are not consistently enforced.',
            highRisk: 'Makers or project users hold tenant-wide admin roles in production without separation of duties.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        helperText: 'Prevents overexposure and access gaps by ensuring the agent grounds only on sources that intended users are both authorized and able to access.',
        isBlocker: true,
        examples: {
            good: 'Connected knowledge sources enforce user/group permissions that match the intended agent audience before go-live.',
            watch: 'Most sources are permission-aligned, but some pilot connectors still expose broader access than intended.',
            highRisk: 'The agent can retrieve content from sources that target users should not be able to access directly.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        helperText: 'Reduces compliance and exposure risk by ensuring grounding sources stay in approved regions and security boundaries.',
        isBlocker: true,
        examples: {
            ...
            watch: 'Most sources meet residency and security requirements, but one or more pilot sources still require policy exceptions or remediation.',
            highRisk: 'Knowledge sources used for grounding violate required residency constraints or lack baseline security protections.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Data is usable but includes stale pages, inconsistent metadata, or duplicate records that degrade response quality.',
            highRisk: 'Grounding includes outdated or unreviewed content that can produce inaccurate answers in critical workflows.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'DLP policies exist, but exceptions allow broad connector combinations or unmanaged sharing in pilot environments.',
            highRisk: 'High-risk configuration: pilot environment allows anonymous/public access while DLP policies permit unrestricted connector data sharing.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Approval controls are defined for some actions, but destructive or compliance-sensitive flows still execute automatically.',
            highRisk: 'Agents can trigger high-impact actions (such as account, financial, or records changes) without human approval gates.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Core connectors are tested, but edge-case integrations or throttling limits are not validated before rollout.',
            highRisk: 'New connectors are first exercised in production using broad credentials and untested data paths.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Environment separation exists, but release promotion criteria and rollback runbooks are incomplete.',
            highRisk: 'A single shared/default environment is used for build, test, and production workloads without promotion controls.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Owners are named, but KPI baselines or review cadence are inconsistent across use cases.',
            highRisk: 'No accountable owner is assigned, so KPI drift and unresolved issues accumulate after launch.',
        },
        recommendedControls: [
    ...
    },
    {
        ...
        examples: {
            ...
            watch: 'Runbook exists, but escalation ownership or response time expectations are not consistently defined.',
            highRisk: 'No incident runbook or escalation path exists, causing delayed containment and recovery during outages.',
        },
        recommendedControls: [
...
function scoreReadinessAnswer(questionId, answer) {
    if (questionId === 'identity-1' && answer === 'na-anonymous-agent')
        return 100;
    if (answer === 'yes')
        return 100;
    if (answer === 'partial')
...
function normalizeWeights(weights) {
    ...
    if (!total)
        return portfolioDefaultWeights;
    return {
...
function scorePortfolioUseCase(useCase, weights) {
    ...
    const positiveRaw = useCase.ratings.businessValue * weights.businessValue +
        useCase.ratings.feasibility * weights.feasibility +
        useCase.ratings.timeToValue * weights.timeToValue;
    const negativeRaw = useCase.ratings.risk * weights.risk + useCase.ratings.dataSensitivity * weights.dataSensitivity;
...
try {
    aiRecommendationService = new AIRecommendationService(process.env.AZURE_OPENAI_API_KEY || process.env.OPENAI_API_KEY || process.env.GITHUB_TOKEN, process.env.AZURE_OPENAI_ENDPOINT || process.env.OPENAI_ENDPOINT, process.env.AZURE_OPENAI_DEPLOYMENT || process.env.OPENAI_MODEL);
...
app.use((_req, res, next) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; base-uri 'self'; connect-src 'self'; font-src 'self' data:; frame-ancestors 'none'; img-src 'self' data: https:; object-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'");
    if (process.env.NODE_ENV === 'production') {
...
app.use((error, _req, res, next) => {
    ...
    if (error?.type === 'entity.parse.failed' || error instanceof SyntaxError) {
        return res.status(400).json({
            ...
            message: 'Request body contains malformed JSON. Check for invalid escape sequences (for example bad Unicode escapes like \\u).',
...
app.get('/api/health', (_req, res) => {
    res.json({
        ...
        features: {
            aiEnabled: !!(process.env.AZURE_OPENAI_ENDPOINT ||
...
app.post('/api/portfolio/prioritize', (req, res) => {
    ...
    const valid = useCases.every((item) => item &&
        ...
        item.ratings &&
        portfolioCriteria.every((criterion) => Number.isFinite(Number(item.ratings[criterion.id]))));
    if (!valid) {
        return res.status(400).json({
            error: 'Invalid use cases',
    ...
    const prioritized = useCases
        ...
        const normalized = {
            ...item,
            ratings: Object.fromEntries(portfolioCriteria.map((criterion) => [
                criterion.id,
    ...
    try {
        ...
        if (!answers || typeof answers !== 'object') {
            return res.status(400).json({
                ...
                message: 'Missing answers. Expected: { answers: { [questionId]: yes|partial|no } } (identity-1 also supports na-anonymous-agent).',
            });
        }
        ...
        const domainScores = readinessDomains.map((domain) => {
            ...
            const total = domainQuestions.reduce((sum, q) => sum + scoreReadinessAnswer(q.id, answers[q.id]), 0);
...
app.post('/api/explain', async (req, res) => {
    try {
        ...
        const aiEnabled = !!process.env.AZURE_OPENAI_ENDPOINT ||
            !!process.env.OPENAI_API_KEY ||
            !!process.env.GITHUB_TOKEN;
        if (!aiEnabled) {
...
app.get('/api/release-planner/copilot-studio', async (_req, res) => {
    try {
...
app.get('/api/foundry/news', async (_req, res) => {
    try {
    ...
    try {
        ...
        if (history !== undefined &&
            ...
            return res.status(400).json({
                ...
                message: 'history must contain at most 10 user or assistant messages of up to 4,000 characters each.',
            });
        }
        // Check if AI is enabled
...
async function generateAIExplanation(recommendation, userContext) {
    ...
    console.log(`[AI Explanation] AI configured: ${aiConfigured} (Azure: ${!!azureEndpoint}, OpenAI/GitHub: ${!!openaiKey})`);
    ...
    try {
        if (!metricsService.isEnabled()) {
            ...
            return res.json({
                ...
                message: 'Metrics integration not configured. Showing sample data. To enable, set AZURE_TENANT_ID, AZURE_CLIENT_ID, and AZURE_CLIENT_SECRET environment variables.',
            });
        }
